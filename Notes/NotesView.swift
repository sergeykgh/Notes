		...
		NavigationView {
			VStack {
				...
				if self.model.pushed {
					NavigationLink(destination: NoteDetailView(isNewNote: true, id: 0, title: ""), isActive: $model.pushed) {
						EmptyView()
					}
				}
			}.onAppear() {
...
struct NoteCell: View {
	let note: Note
	var body: some View {
		return NavigationLink(destination: NoteDetailView(isNewNote: false, id: note.id, title: note.title)) {
			Text(note.title)
				.frame(height: 60)
				.lineLimit(2)
		}
